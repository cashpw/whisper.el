...
(add-hook 'whisper-post-process-hook
  ...
  #+end_src
- =whisper-post-transcription-hook=: These functions are run after transcription is completed and the text has been inserted into the buffer. For example, you can pipe the output into another function:
  #+begin_src elisp
